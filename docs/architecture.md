...
Backpack Cloud is another inference route behind the local API, not a `ComputeTarget` pretending to own local sessions. A `:cloud` request is resolved against the live hosted model list and forwarded in its native Chat Completions, Responses, or Messages shape. A random per-daemon credential authenticates the local agent-to-runtime hop; the runtime alone holds and applies the upstream API key or short-lived device token.
