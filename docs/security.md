...
- Codex App configuration never reads or changes `auth.json`. It stores the exact pre-Backpack TOML in a private backup, rejects symlinked/non-regular config files, validates TOML before and after patching, and refuses automatic restore after config drift. Desktop requests use an unguessable per-daemon token embedded in a loopback-only route; the app's authorization header is replaced before any Cloud request.
