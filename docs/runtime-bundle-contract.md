...
The packager's current `runtime.yaml`/`runtime_services` contract remains appropriate for model-specific protocol workers such as Qwen ASR and the current Whisper worker. A future backward-compatible packager revision should emit the normalized top-level requirement directly and should reference shared native bundles instead of attaching the same llama.cpp binary to every GGUF repository.
