...
The launch feature is experimental. The compatibility protocols and the installed Codex executable have passed isolated protocol/tool-loop tests, but no Backpack coding model has yet completed the full real-model agent qualification. Claude Code was not installed on the qualification host.
...
Backpack configures provider routing, model selection, and an isolated configuration directory. It does not weaken Codex sandbox/approval settings or Claude Code permissions, and never adds permission-bypass flags. Tool execution, workspace access, and user approvals remain owned by the launched agent.
