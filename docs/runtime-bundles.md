...
Runtime bundles and the `backpack` CLI release are separate signed-ready artifacts. End users install one versioned CLI archive; the trusted runtime catalog then selects and verifies engine bundles as needed. Windows x64 has the broadest execution validation, and the Linux amd64 CPU GGUF release path is qualified. Other Linux runtime families and macOS arm64 remain experimental until real execution succeeds; build portability alone is not a support claim.
